class VoiceConditionLogAnalysisChannel < ApplicationCable::Channel
  def subscribed
    ...
    # 例： consumer.subscriptions.create({ channel: "VoiceConditionLogAnalysisChannel", id: voiceConditionLogId }, { ... })
    ...
    if voice_condition_log
      ...
      reject # 購読を拒否
      Rails.logger.warn "Client failed to subscribe: VoiceConditionLog with ID #{params[:id]} not found or unauthorized."
    end
  end
  ...
  def unsubscribed
    ...
    Rails.logger.info "Client unsubscribed from VoiceConditionLogAnalysisChannel (was streaming for ID #{params[:id] if params[:id]})"
  end
end